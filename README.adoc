...
A simple template engine based loosely on the FreeMarker syntax. FreshMarker is implemented in Java 17 and supports the java.time API and Records.
...
[source,java]
----
templateLoader.putTemplate("ancestors-loop",
    """
...
== FreshMarker List Syntax
[code]
----
<#list ancestors as ancestor, looper>
  ${looper.counter} ${ancestor.givenname} ${ancestor.surname}
</#list>
----