...
The elements of sequences can be accessed via the hash operator `sequence[_index_]` or the <<_list_slices,slice>> operator.

.Example Sequence hash operators
...
Ranges differ from other types because they only exists as literals in the templates. Ranges cannot be injected via the model.
