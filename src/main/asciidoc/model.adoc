...
_Sequences_ represent all data structures that implement `java.util.List`.
...
NOTE: Up to version 1.10.1, only primitive values could be specified in hash literals.
This can be configured now via `SystemFeature.NON_PRIMITIVE_HASH_LITERAL_VALUES`. See <<system-feature>>.
...
=== Forbidden Types (deprecated since 1.10.0)

CAUTION: As FreshMarker does not execute any methods on model classes, there is no risk of injection. This mechanism will therefore be removed with version 2.0.0.
