...
| Feature that determines whether a duplicate case expression results in an error (since 1.11.0).
| `false`
|===
...
| Feature that determines whether the hash literal could contain non-primitive values. Prior version 1.11.0 only primitive values are allowed.
| `false`
