...
| Feature that determines whether the switch optimized with a map (since 1.10.1).
| `false`

| *ERROR_ON_DUPLICATE_CASE_EXPRESSION*
| Feature that determines whether a duplicate case expression results in an error (since 1.10.1).
| `false`
|===
...
CAUTION: This feature is deprecated and the feature `SystemFeature.ALL_BLOCKS` should be used. See <<system-feature>>

This feature is controlled by a constant in the `VariableScopeFeature` class.
...
| *ALL_BLOCKS*
| Feature that allows variable contexts at every directive. Prior version 1.9.0 only list, macro and conditional directives span a variable context.
| `false`
|===
...
[#system-feature,reftext=System Features]
===== System Features (since 1.10.0)

A constant controls this feature in the `SystemFeature` class.

CAUTION: These features will be removed in the version 2.0.0, and the default enabled implementation is removed too.
...
| Feature that determines whether the hash literal could contain non-primitive values. Prior version 1.10.1 only primitive values are allowed.
| `false`
